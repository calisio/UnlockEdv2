...
func TestHandleIndexLibraries(t *testing.T) {
	httpTests := []httpTest{
		{"TestGetLibrariesAsUser", "student", map[string]any{"page": 1, "per_page": 10, "visibility": "visible", "search": "", "provider_id": 0, "user_id": uint(7)}, http.StatusOK, "?page=1&per_page=10"},
		{"TestGetLibrariesAsUserShowHidden", "student", map[string]any{"page": 1, "per_page": 10, "visibility": "hidden", "search": "", "provider_id": 0, "user_id": uint(7)}, http.StatusUnauthorized, "?page=1&per_page=10&visibility=hidden"},
